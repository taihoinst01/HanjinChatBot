    ...
    public class DbConnect
    {
        ...
        public int insertHistory(List<RelationList> relationList, string userNumber, string channel, int responseTime, string luis_intent, string luis_entities, string luis_intent_score, string dlg_id, string reply_result, string queryStr)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connStr))
            {
                ...
                cmd.CommandText += " (@userNumber, @customerCommentKR, @chatbotCommentCode, @channel, @responseTime, CONVERT(VARCHAR,  DATEADD( HH, 9, GETDATE() ), 101) + ' ' + CONVERT(VARCHAR,  DATEADD( HH, 9, GETDATE() ), 24), 0, @appID, @luis_intent, @luis_entities, @luis_intent_score, @dlg_id, @result, (SELECT TOP 1 USER_ID FROM TBL_USERDATA WHERE CHANNELDATA=@channel AND CONVERSATIONSID = @userNumber), (SELECT TOP 1 MOBILEPC FROM TBL_USERCHECK WHERE CHANNELDATA=@channel AND CONVERSATIONSID = @userNumber)) ";
